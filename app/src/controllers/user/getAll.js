...
const { getAllUsers: getAllUsersService } = require('../../services');
...
module.exports = async function getAllUsers(req, res, next) {
  try {
    ...
    const result = await getAllUsersService(validated.page, validated.size);
    if (!result.success) {
      return sendResponse(res, StatusCodes.INTERNAL_SERVER_ERROR, false, result.message);
    }
    return sendResponse(res, StatusCodes.OK, true, result.message, { users: result.data });
  } catch (error) {
    next(error);
  }
};
